...
연결 리스트(LinkedList)
    ...
    한 방향으로만 탐색이 가능한 구조이다.
'''
class Node:
    def __init__(self, data, next=None):
        self.data = data # 노드에 저장될 데이터
        self.next = next # 다음 노드를 가리키는 포인터(주소값)
    
class LinkedList:
    def __init__(self):
        self.head = None # 미리 노드를 저장하는 멤버 변수
    ...
    def delete_node(self, del_data):
        if self.head is None: # 언결 리스트가 비어 있는 경우
            return
        ...
        current = self.head
        while current.next is not None: # 연결 리스트를 따라가며 원하느 노드 찾기
    ...
    def print_list(self):
        current = self.head
        while current is not None: # 연결 리스트를 따라가며 데이터를 출력
...
linkedList.insert_node(9, 99) # 9 다음에 99삽입
